...
export const Users = () => {
  ...
  const MenuButtons = [
    { title: "New", icon: PlusIcon, variant: "gradient", click:() => {handleDialogOpen(0)}},
    { title: "Upload File", icon: CloudArrowUpIcon ,variant:"gradient",click:() => {handleDialogOpen(3)}},
  ];
    ...
    const actionMenuItems = [
      {
        ...
        click: () => { handleDialogOpen(1,row.original) },
      },
      {
        label: "Delete",
        icon: TrashIcon,
        click: () => { handleDialogOpen(2,row.original)},
      },
    ];
    return (
  ...
  return (
    ...
    <div className="my-20 flex flex-col gap-12">
      <TableBuild title={title} Menu={MenuButtons} columns={columns} data={usersData} />
      </div>
      <Dialog open={isOpenDialog} handler={() => setUserDialog({ open: false })} size="sm">
